...
/// Unified message list renderer — handles both AllInboxes and SingleFolder contexts.
fn render_message_list(frame: &mut Frame, app: &App) {
    ...
    let section_starts: std::collections::HashMap<usize, (&str, bool)> = match &app.folder_context {
        FolderContext::AllInboxes => {
            if searching {
                let mut map = std::collections::HashMap::new();
                let mut first = true;
                for section in &app.sections {
                    let first_visible = (section.start..section.start + section.count)
                        .find(|i| visible_set.contains(i));
        ...
        FolderContext::SingleFolder { account_key, .. } => {
            // Single account header at top
            if !visible_indices.is_empty() {
                let first_idx = visible_indices[0];
                let mut map = std::collections::HashMap::new();
                map.insert(first_idx, (account_key.as_str(), true));
                map
            } else {
    ...
    let title = format!(" {} ", app.folder_display_name());
